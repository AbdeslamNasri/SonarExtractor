...
public class DefaultQualityGateClient implements QualityGateClient {
  ...
  private static final String PROJECT_STATUS_URL = ROOT_URL + "/project_status";
  ...
  @Override
  public QualityGateDetailsProject projectStatus(String projectKey) {
    String json = requestFactory.get(PROJECT_STATUS_URL, Collections.singletonMap("projectKey", (Object) projectKey));
    return jsonToDetailsProject(json);
  }
  ...
  @SuppressWarnings({"rawtypes", "unchecked"})
  private QualityGateDetailsProject jsonToDetailsProject(String json) {
    Map jsonRoot = (Map) JSONValue.parse(json);
    Map json2 = (Map) jsonRoot.get("projectStatus");
    return new DefaultQualityGateDetailsProject(json2, jsonToConditionsProject(json));
  }
  ...
  @SuppressWarnings({"rawtypes", "unchecked"})
  private Collection<QualityGateConditionProject> jsonToConditionsProject(String json) {
    Map jsonRoot = (Map) JSONValue.parse(json);
    Map json2 = (Map) jsonRoot.get("projectStatus");
    Collection<Map> conditionArray = (Collection<Map>) json2.get("conditions");
    Collection<QualityGateConditionProject> conditions = new ArrayList<QualityGateConditionProject>();
    if (conditionArray != null) {
      for (Map conditionJson: conditionArray) {
        conditions.add(new DefaultQualityGateConditionProject(conditionJson));
      }
    }
    return conditions;
  }
